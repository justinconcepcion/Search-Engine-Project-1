...
public class TermDocumentIndexer {
	...
	private static void indexFile(Path file, List<String> fileNames, TermDocumentIndex index,
	...
	public static void main(String[] args) {
		...
		if (postings.size() != 0) {
			...
			for (int j = 0; j < postings.size(); j++) {
				System.out.println("Document ID " + postings.get(j).getDocumentId());
			}
		} else {
			System.out.println("There is no term available. Please run the program again.");
		}
		reader.close();
	}
}
